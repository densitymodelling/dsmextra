...
compare_covariates(extrapolation.type = "both",
                   covariate.names = covariates.ziphius,
                   n.covariates = NULL,
                   samples = segments.ziphius$transformed, 
                   prediction.grid = predgrid.ziphius$transformed,
                   coordinate.system = aftt_crs,
                   create.plots = TRUE,
                   display.percent = TRUE)
...
#'---------------------------------------------
ziphiids.nearby <- list(transformed = NULL, untransformed = NULL)

ziphiids.nearby$untransformed <- compute_nearby(samples = segments.ziphiids$untransformed, 
...
ziphiids.nearby$transformed <- compute_nearby(samples = segments.ziphiids$transformed, 
...
#'---------------------------------------------
ziphiids.nearby$difference <- ziphiids.nearby$transformed - ziphiids.nearby$untransformed
```

```{r eval=TRUE, message=FALSE, warning=FALSE, include=FALSE, echo=FALSE}
ziphius.nearby <- list(transformed = NULL, untransformed = NULL)
ziphius.nearby$untransformed <- raster::raster(file.path("/Users/philippebouchet/Dropbox/bouchet-extrapolation", "data", "outputs", "ziphiids_nearby_untransformed.tif"))
ziphius.nearby$transformed <- raster::raster(file.path("/Users/philippebouchet/Dropbox/bouchet-extrapolation", "data", "outputs", "ziphiids_nearby_transformed.tif"))
# Calculate the difference between the two rasters
...
map_extrapolation(map.type = "extrapolation", 
                  extrapolation.values = ziphius.extrapolation$untransformed,
                  covariate.names = covariates.ziphius,
                  prediction.grid = predgrid.ziphius$untransformed, 
                  coordinate.system = aftt_crs,
                  base.layer = "gray")
...
map_extrapolation(map.type = "extrapolation",
                  extrapolation.values = ziphius.extrapolation$transformed,
                  covariate.names = covariates.ziphius,
                  prediction.grid = predgrid.ziphius$transformed, 
                  coordinate.system = aftt_crs,
                   base.layer = "gray")
...
map_extrapolation(map.type = "nearby",
                  gower.values = ziphius.nearby$untransformed,
                  covariate.names = covariates.ziphius,
                  prediction.grid = predgrid.ziphius$untransformed, 
                  coordinate.system = aftt_crs)

map_extrapolation(map.type = "nearby",
                  gower.values = ziphius.nearby$transformed,
                  covariate.names = covariates.ziphius,
                  prediction.grid = predgrid.ziphius$transformed, 
                  coordinate.system = aftt_crs)
```
...
```{r echo=TRUE, message=FALSE, warning=FALSE, fig.cap=" "}
map_extrapolation(map.type = "nearby",
                  gower.values = ziphius.nearby$difference,
                  covariate.names = covariates.ziphius,
                  prediction.grid = predgrid.ziphius$transformed, 
                  coordinate.system = aftt_crs)

breakpoints <- c(-35,-0.5,0.5,10)
plot(ziphius.nearby$difference, breaks = breakpoints, col = pals::viridis(3))
```
